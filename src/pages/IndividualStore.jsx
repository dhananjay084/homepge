...
//   const ListItems = ['Beauty & Spa', 'Things to do', 'Auto & Home'];
  ...
  const { chartData, todayCount } = useMemo(() => {
    ...
    const counts = dateLabels.map(({ date, label }) => {
      const count = deals.filter(deal => {
        ...
        if (isSameDay && isSameStore && date === now.toISOString().split('T')[0]) {
          todayCount++;
        }
  ...
  if (loading || !storeFromList) return <p className="text-center py-10">Loading...</p>;
  if (error) return <p className="text-red-500 text-center py-10">{error}</p>;

  return (
    <div>
      <TextLink text={storeFromList.storeName} colorText="Offers" link="" linkText="" />
     
      <div className='border-b pb-2 mb-4 px-4'>
      <Typography sx={{ fontSize: '13px' }}>{storeFromList.storeDescription}</Typography>

        {/* <p>{storeFromList.storeDescription}</p> */}
      ...
      <div className='bg-[#F6F6F6] border border-[#E1E1E1] rounded-lg p-4 mx-2 flex justify-between items-center sm:max-w-[80%] sm:mx-auto'>
        ...
        <div  className="pt-3 flex justify-between mx-2 gap-4 sm:max-w-[80%] sm:mx-auto">
          <div className='bg-[#F6F6F6] border border-[#E1E1E1] rounded-lg p-4 w-full'>
            <p>Total Offers</p>
            <p className='text-xl font-semibold text-[#592EA9]'>{`${chartData.reduce((sum, d) => sum + d.value, 0)} `}</p>
          </div>
          <div className='bg-[#F6F6F6] border border-[#E1E1E1] rounded-lg p-4 w-full'>
            <p>Biggest discount</p>
            <p className='text-xl font-semibold text-[#592EA9]'>{storeFromList.discountPercentage}%</p>
          </div>
        </div>
